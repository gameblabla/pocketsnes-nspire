...
/*
  ...
  */
void FxPipeString_text(char * pvString_text)
{
    char *p;
    ...
    sprintf(pvString_text, "%02x:%04x %02x       ",
	    USEX8(vPipeBank), USEX16(GSU.vPipeAdr), USEX8(PIPE));
    p = &pvString_text[strlen(pvString_text)];
 
    /* Check if it's a branch instruction */
    if( PIPE >= 0x05 && PIPE <= 0x0f )
    {
	sprintf(&pvString_text[11], "%02x    ", USEX8(vPipe1));
    ...
    {
	sprintf(&pvString_text[11], "%02x %02x ", USEX8(vPipe1), USEX8(vPipe2));
