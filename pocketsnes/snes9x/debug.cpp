...
void ProcessDebugCommand (char *Line)
{
	...
	{
	    sprintf (String_text, "%06x%05d.sd2", Address, Count);
	    FILE *fs = fopen (String_text, "wb");
	    if (fs)
	    {
		int i;
		for (i = 0; i < Count; i++)
    ...
    {
	printf ("Vectors:\n");
	sprintf (String_text, "      8 Bit   16 Bit ");
	DPrint (String_text);
	sprintf (String_text, "ABT $00:%04X|$00:%04X", S9xGetWord (0xFFF8), S9xGetWord (0xFFE8));
	DPrint (String_text);
	sprintf (String_text, "BRK $00:%04X|$00:%04X", S9xGetWord (0xFFFE), S9xGetWord (0xFFE6));
	DPrint (String_text);
	sprintf (String_text, "COP $00:%04X|$00:%04X", S9xGetWord (0xFFF4), S9xGetWord (0xFFE4));
	DPrint (String_text);
	sprintf (String_text, "IRQ $00:%04X|$00:%04X", S9xGetWord (0xFFFE), S9xGetWord (0xFFEE));
	DPrint (String_text);
	sprintf (String_text, "NMI $00:%04X|$00:%04X", S9xGetWord (0xFFFA), S9xGetWord (0xFFEA));
	DPrint (String_text);
	sprintf (String_text, "RES     $00:%04X", S9xGetWord (0xFFFC));
	DPrint (String_text);
    }
    ...
    if (Line[0] == 's')
    {
	CPU.PC += S9xOPrint (String_text, Bank, Address);
	Bank = Registers.PB;
...
//	printf ("\n");
	...
	*Line = 0;
	...
	Address += S9xOPrint (String_text, Bank, Address);
	if (strncmp (&String_text[18], "JMP", 3) != 0 &&
		strncmp (&String_text[18], "JML", 3) != 0 &&
		strncmp (&String_text[18], "RT", 2) != 0 &&
		strncmp (&String_text[18], "BRA", 3))
	{
	...
	if (Line[1] == 'v')
	{
		...
		for (Number = 0; Number != 5; Number++)
		{
			...
			sprintf (String_text, "%i @ $%02X:%04X", Number, S9xBreakpoint[Number].Bank, S9xBreakpoint[Number].Address);
		    else
			sprintf (String_text, "%i @ Disabled", Number);
		    DPrint (String_text);
		}
	    }
	    else
	...
	{
	    sprintf (String_text, "$%02X:%04X", Bank, Address);
		...
		sprintf (String_text, "%s %02X", String_text, MemoryByte);
	    }
	    sprintf (String_text, "%s-", String_text);
	    for (CByte = 0; CByte != 16; CByte++)
	    {
		if (Address + CByte == 0x2140 ||
		    Address + CByte == 0x2141 ||
		...
		if (MemoryByte < 32 || MemoryByte >= 127)
		    MemoryByte = '?';
		sprintf (String_text, "%s%c", String_text, MemoryByte);
	    }
	    Address += 16;
	    DPrint (String_text);
	}
	...
	DPrint (String_text);
#endif	
	S9xOPrint (String_text, Bank, Address);
	DPrint (String_text);
    }
    if (Line[0] == 'u')
    {
...
void S9xTrace ()
{
    ...
    char String_text [512];
    S9xOPrint (String_text, Registers.PB, CPU.PC - CPU.PCBase);
    fprintf (trace, "%s\n", String_text);
}
...
{
    char String_text [512];
    S9xSA1OPrint (String_text, SA1Registers.PB, SA1.PC - SA1.PCBase);
    fprintf (trace2, "%s\n", String_text);
    fflush (trace2);
}
