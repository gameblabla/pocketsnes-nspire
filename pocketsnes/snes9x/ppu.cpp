		...
		if (Settings.SA1)
		{
			...
			if (Address == 0x2801 && Settings.SRTC)
				...
				if (Address < 0x3000 || Address >= 0x3300)
				{
					...
					{
						sprintf (String_text, "Unknown register write: $%02X->$%04X\n",
								 Byte, Address);
						S9xMessage (S9X_TRACE, S9X_PPU_TRACE, String_text);
					}
#endif
				}
				else
...
//		CPU.WaitAddress = CPU.PCAtOpcodeStart;
	...
	if (Address <= 0x2fff || Address >= 0x3300)
	{
	    ...
	    default:
	        ...
	        if (Settings.TraceUnknownRegisters)
		{
		    sprintf (String_text, "Unknown register read: $%04X\n", Address);
		    S9xMessage (S9X_TRACE, S9X_PPU_TRACE, String_text);
		}
#endif
		return OpenBus;
	...
	if (Address < 0x4200)
	{
		...
		switch (Address)
		{
		  ...
		  default:
			...
			if (Settings.TraceUnknownRegisters)
			{
				sprintf (String_text, "Unknown register register write: $%02X->$%04X\n",
						 byte, Address);
				S9xMessage (S9X_TRACE, S9X_PPU_TRACE, String_text);
			}
#endif
