...
//			Settings.MultiPlayer5Master = FALSE;
	...
	sprintf (CompanyId, "%s", Safe (CompanyId));
		...
		sprintf (String_text, "\"%s\" [%s] %s, %s, Type: %s, Mode: %s, TV: %s, S-RAM: %s, ROMId: %s Company: %2.2s CRC32: %08X",
		ROMName,
	...
	S9xMessage (S9X_INFO, S9X_ROM_INFO, String_text);
#ifdef __WIN32__
	#ifndef _XBOX
...
const char *CMemory::KartContents ()
{
    ...
    snprintf (tmp, sizeof(tmp), "%s", Contents [(ROMType & 0xf) % 3]);
	

	if(Settings.BS)
	{
		snprintf (previous_tmp, sizeof(previous_tmp), "%s", tmp);
		snprintf (tmp, sizeof(tmp), "%s+%s", previous_tmp, "BSX");
	}
	else if(Settings.SPC7110&&Settings.SPC7110RTC)
	{
		snprintf (previous_tmp, sizeof(previous_tmp), "%s", tmp);
		snprintf (tmp, sizeof(tmp), "%s+%s", previous_tmp, "SPC7110+RTC");
	}
	else if(Settings.SPC7110)
	{	
		snprintf (previous_tmp, sizeof(previous_tmp), "%s", tmp);
		snprintf (tmp, sizeof(tmp), "%s+%s", previous_tmp, "SPC7110");
	}
	else if(Settings.SETA!=0)
	{
		...
		case ST_010:
			snprintf (previous_tmp, sizeof(previous_tmp), "%s", tmp);
			snprintf (tmp, sizeof(tmp), "%s+%s", previous_tmp, "ST-010");
			break;
		case ST_011:
			snprintf (previous_tmp, sizeof(previous_tmp), "%s", tmp);
			snprintf (tmp, sizeof(tmp), "%s+%s", previous_tmp, "ST-011");
			break;

		case ST_018:
			snprintf (previous_tmp, sizeof(previous_tmp), "%s", tmp);
			snprintf (tmp, sizeof(tmp), "%s+%s", previous_tmp, "ST-018");
			break;
    ...
    {
		snprintf (previous_tmp, sizeof(previous_tmp), "%s", tmp);
		snprintf (tmp, sizeof(tmp), "%s+%s", previous_tmp, CoPro [(ROMType & 0xf0) >> 4]);
	}
